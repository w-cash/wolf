...
impl CopyStateCmd {
    ...
    async fn start(&self) -> Result<(), Report> {
        ...
        //
        // Use the `WCASH_TARGET_` environment prefix for target overrides to avoid
        // conflicting with the source/base config (`WCASH_...`).
        // Example: `WCASH_TARGET_STATE__CACHE_DIR=/dst/cache`.
        let target_config = self
            .target_config_path
            .as_ref()
            .map(|path| {
                crate::config::ZebradConfig::load_with_env(Some(path.clone()), "WCASH_TARGET")
            })
            .transpose()?

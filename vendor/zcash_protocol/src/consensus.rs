...
pub trait Parameters: Clone {
    ...
    /// Returns the consensus branch ID used for transactions in `nu`.
    ///
    /// Zcash networks use the branch ID assigned to the network upgrade. Downstream
    /// protocols that reuse a Zcash rule set on an independent chain must override
    /// this method with their own globally unique branch ID, so transaction
    /// signatures cannot be replayed between the chains.
...
impl NetworkUpgrade {
    ...
    ///
    /// This is the standard Zcash mapping. Independent protocols can override it
    /// through [`Parameters::branch_id_for_upgrade`].
...
pub enum BranchId {
    ...
    Nu6_3,
    /// Wcash Testnet v1 rules, which use NU6.3 / Ironwood semantics with a
    /// chain-specific transaction signature domain.
    ///
    ...
    /// Wcash Testnet.
    WcashTestnetV3,
    /// Wcash Mainnet v1 rules, which use NU6.3 / Ironwood semantics with a
    /// transaction signature domain independent from Zcash and Wcash Testnet.
    WcashMainnetV1,
...
impl TryFrom<u32> for BranchId {
    ...
    fn try_from(value: u32) -> Result<Self, Self::Error> {
        match value {
            ...
            0xd9c6_a7ee => Ok(BranchId::WcashMainnetV1),
...
impl From<BranchId> for u32 {
    fn from(consensus_branch_id: BranchId) -> u32 {
        match consensus_branch_id {
            ...
            BranchId::WcashMainnetV1 => 0xd9c6_a7ee,
...
impl BranchId {
    ...
    pub fn for_height<P: Parameters>(parameters: &P, height: BlockHeight) -> Self {
        for nu in UPGRADES_IN_ORDER.iter().rev() {
            if parameters.is_nu_active(*nu, height) {
                return parameters.branch_id_for_upgrade(*nu);
            }
        }
    ...
    ///
    /// Multiple independently domain-separated branch IDs can use the same network
    /// upgrade semantics, so this mapping is not necessarily one-to-one.
    ...
    pub fn orchard_protocol_revision(&self) -> Option<OrchardProtocolRevision> {
        ...
        match self {
            ...
            Nu6_3 | WcashTestnetV1 | WcashTestnetV3 | WcashMainnetV1 | WcashRegtestV1 => {
...
mod tests {
    use super::{
        BlockHeight, BranchId, MAIN_NETWORK, NetworkType, NetworkUpgrade, Parameters,
        UPGRADES_IN_ORDER,
    };
    ...
    #[derive(Clone)]
    struct WcashRegtestV1;

    impl Parameters for WcashRegtestV1 {
        fn network_type(&self) -> NetworkType {
            NetworkType::Regtest
        }
    ...
    #[derive(Clone)]
    struct WcashMainnetV1;

    impl Parameters for WcashMainnetV1 {
        fn network_type(&self) -> NetworkType {
            NetworkType::Main
        }
        ...
        fn branch_id_for_upgrade(&self, nu: NetworkUpgrade) -> BranchId {
            match nu {
                NetworkUpgrade::Nu6_3 => BranchId::WcashMainnetV1,
                _ => nu.branch_id(),
            }
        }
    }
    ...
    fn branch_id_from_u32() {
        ...
        assert_eq!(
            BranchId::try_from(0xb3cf_d27e),
            Ok(BranchId::WcashTestnetV1)
        );
        ...
        assert_eq!(
            BranchId::try_from(0xd9c6_a7ee),
            Ok(BranchId::WcashMainnetV1)
        );
        assert_eq!(u32::from(BranchId::WcashMainnetV1), 0xd9c6_a7ee);
        assert_eq!(
            BranchId::try_from(0xc3a6_678a),
            Ok(BranchId::WcashRegtestV1)
        );
        assert_eq!(u32::from(BranchId::WcashRegtestV1), 0xc3a6_678a);
    ...
    fn wcash_testnet_v3_has_an_independent_transaction_domain() {
        ...
        assert_eq!(
            BranchId::for_height(&MAIN_NETWORK, BlockHeight::from_u32(3_428_143)),
            BranchId::Nu6_3
        );
        assert_ne!(
    ...
    #[test]
    fn wcash_regtest_v1_has_an_independent_transaction_domain() {
        let params = WcashRegtestV1;
        ...
        assert_eq!(
            BranchId::WcashRegtestV1.network_upgrade(),
            Some(NetworkUpgrade::Nu6_3)
        );
    ...
    #[test]
    fn wcash_mainnet_v1_has_an_independent_transaction_domain() {
        let params = WcashMainnetV1;
        ...
        assert_eq!(
            BranchId::WcashMainnetV1.network_upgrade(),
            Some(NetworkUpgrade::Nu6_3)
        );
        assert!(BranchId::WcashMainnetV1.height_bounds(&params).is_some());
        assert!(BranchId::Nu6_3.height_bounds(&params).is_none());
        assert_ne!(
            u32::from(BranchId::WcashMainnetV1),
            u32::from(BranchId::WcashTestnetV3)
        );
        assert_ne!(
            u32::from(BranchId::WcashMainnetV1),
    ...
    fn orchard_protocol_revision() {
        ...
        assert_eq!(
            BranchId::WcashTestnetV1.orchard_protocol_revision(),
            Some(OrchardProtocolRevision::V3)
        );
        assert_eq!(
        ...
        assert_eq!(
            BranchId::WcashMainnetV1.orchard_protocol_revision(),
            Some(OrchardProtocolRevision::V3)
        );
        assert_eq!(
            BranchId::WcashRegtestV1.orchard_protocol_revision(),
            Some(OrchardProtocolRevision::V3)
        );
        #[cfg(zcash_unstable = "nu7")]
        assert_eq!(

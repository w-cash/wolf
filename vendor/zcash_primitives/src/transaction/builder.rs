...
/// An explicitly selected transaction branch ID did not match the branch that
/// the supplied consensus parameters activate at the target height.
...
impl BranchIdMismatch {
    /// Returns the branch selected by the consensus parameters and target height.
    ...
    fn check_version_compatibility<FE>(&self, version: TxVersion) -> Result<(), Error<FE>> {
...

        {
            ...
            let ironwood_branch = match self.consensus_branch_id {
                BranchId::Nu6_3 | BranchId::WcashTestnetV1 | BranchId::WcashTestnetV3 | BranchId::WcashMainnetV1 | BranchId::WcashRegtestV1 => true,
...
impl<P: consensus::Parameters> Builder<P, ()> {
    ...
    ///
    /// This constructor is intended for callers that cross a chain boundary and
    /// therefore need the selected signature domain to be visible at the call site.
    /// It fails closed unless `consensus_branch_id` exactly matches the branch selected
    /// by `params` at `target_height`.
...
mod tests {
    ...
    use {
        ...
        zcash_protocol::{
            consensus::{
                BlockHeight, NetworkType, NetworkUpgrade, Parameters, TEST_NETWORK,
            },
            memo::MemoBytes,
            value::{BalanceError, ZatBalance, Zatoshis},
        },
    ...
    #[cfg(feature = "circuits")]
    #[derive(Clone)]
    struct WcashTestnetV1Parameters;

    #[cfg(feature = "circuits")]
    impl Parameters for WcashTestnetV1Parameters {
    ...
    fn explicit_builder_branch_id_fails_closed() {
        let config = || BuildConfig::Coinbase { miner_data: None };

        let automatic = Builder::new(
            WcashTestnetV1Parameters,
        ...
        .expect("the Wcash branch selected by the parameters must be accepted");

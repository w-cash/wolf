...
use zebra_chain::{
    ...
    parameters::{ConsensusBranchId, Network, WCASH_TESTNET_V3_BRANCH_ID},
...
fn wcash_get_blockchain_info_reports_custom_transaction_domain() {
    ...
    runtime.block_on(async move {
        ...
        assert_eq!(response.consensus.chain_tip.0, WCASH_TESTNET_V3_BRANCH_ID);
        assert_eq!(response.consensus.next_block.0, WCASH_TESTNET_V3_BRANCH_ID);
        assert!(
            response
                .upgrades
                .keys()
                .any(|id| id.0 == WCASH_TESTNET_V3_BRANCH_ID),
            "the upgrades map must expose the Wcash Testnet v3 branch ID"
        );
